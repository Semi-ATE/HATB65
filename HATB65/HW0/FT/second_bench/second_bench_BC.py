...
If you have the need to add things, add it to 'second_bench.py' or 'common.py'
...
class second_bench_OP:
    """Class definition for the output parameters of second_bench."""

    def __init__(self):
        self.num_outputs = 0
        self.Iout = OutputParameter('Iout', -inf, 0.5, 0.5, 2.5, inf, -3, False)
        self.Iout.set_format('.3f')
        self.Iout.set_unit('A')
...
class second_bench_IP:
    """Class definition for the input parameters of second_bench."""
...
class second_bench_BC(DutTestCaseBase):
    '''Base class definition for second_bench'''
    ...
    def __init__(self, instance_name: str, sbin, test_num, context, stil_tool: StilToolBase):
        ...
        self.ip = second_bench_IP()
        self.op = second_bench_OP()
    ...
    def aggregate_test_result(self, site_num: int, exception: bool):
        ...
        test_bin = -1
        if not exception:
            current_result = self.op.Iout.get_testresult()
            test_result = self._select_testresult(test_result, current_result)
            test_bin = self._select_bin(test_bin, current_result)
            stdf_data.append(self.op.Iout.generate_stdf_result_record(current_result[0], site_num))
        else:
            test_result = Result.Fail()
            test_bin = self.get_sbin()
            stdf_data.append(self.op.Iout.generate_stdf_result_record(test_result, site_num))

        return (test_result, test_bin, stdf_data)

    def aggregate_tests_summary(self, head_num: int, site_num: int):
        stdf_data = []
        stdf_data.append(self.op.Iout.generate_tsr_record(head_num, site_num, self.get_average_test_execution_time()))

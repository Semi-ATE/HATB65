...
class first_bench_OP:
    ...
    def __init__(self):
        self.num_outputs = 0
        self.vout = OutputParameter('vout', -inf, 1.0, 1.0, 5.0, inf, 0, False)
        self.vout.set_format('.3f')
        self.vout.set_unit('˽')
...
class first_bench_BC(DutTestCaseBase):
    ...
    def aggregate_test_result(self, site_num: int, exception: bool):
        ...
        test_bin = -1
        if not exception:
            current_result = self.op.vout.get_testresult()
            test_result = self._select_testresult(test_result, current_result)
            test_bin = self._select_bin(test_bin, current_result)
            stdf_data.append(self.op.vout.generate_stdf_result_record(current_result[0], site_num))
        else:
            test_result = Result.Fail()
            test_bin = self.get_sbin()
            stdf_data.append(self.op.vout.generate_stdf_result_record(test_result, site_num))

        return (test_result, test_bin, stdf_data)

    def aggregate_tests_summary(self, head_num: int, site_num: int):
        stdf_data = []
        stdf_data.append(self.op.vout.generate_tsr_record(head_num, site_num, self.get_average_test_execution_time()))
